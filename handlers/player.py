...
@Client.on_message(filters.command(["play", f"play@{BOT_USERNAME}"]) & (filters.chat(CHAT) | filters.private) | filters.audio & filters.private)
async def yplay(_, message: Message):
    ...
    if message.audio:
        ...
        if message.reply_to_message:
            ...
            d=await message.reply_text("لم تعطني أي شيء التشغيله.  أرسل لي ملفًا صوتيًا أو قم بالرد / التشغيل على ملف صوتي.")
            await mp.delete(d)
            await mp.delete(message)
            return
    ...
    if type=="audio":
        if round(m_audio.audio.duration / 60) > DURATION_LIMIT:
            d=await message.reply_text(f"❌ غير مسموح بالصوتيات التي تزيد مدتها عن {DURATION_LIMIT} دقيقة ، والصوت المقدم هو {round (m_audio.audio.duration / 60)} دقيقة (s)")
            await mp.delete(d)
            await mp.delete(message)
            return
        ...
        if len(playlist) == 1:
            m_status = await message.reply_text(
                f"{emoji.INBOX_TRAY} جاري التحميل والمعالجة ..."
            )
    ...
    if type=="youtube" or type=="query":
        if type=="youtube":
            msg = await message.reply_text("⚡️ ** إحضار الأغنية من اليوتيوب ... **")
            url=yturl
        elif type=="query":
            try:
                msg = await message.reply_text("⚡️ ** إحضار الأغنية من اليوتيوب ... **")
        ...
        if int(duration) > DURATION_LIMIT:
            k=await message.reply_text(f"❌ لا يُسمح بمقاطع الفيديو التي تزيد مدتها عن {DURATION_LIMIT} دقيقة ، الفيديو المقدم مدته {duration} دقيقة (s)")
            await mp.delete(k)
            await mp.delete(message)
            return
        now = datetime.now()
...
async def deezer(_, message):
    ...
    if " " in message.text:
        ...
        k=await message.reply_text("أنت لم تعطيني أي شيء التشغيله الاستخدام /splay <song name>")
        await mp.delete(k)
        await mp.delete(message)
        return
    user=f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
    ...
    if album:
        ...
        try:
            ...
            k=await msg.edit("لا يمكن العثور على هذا القائمه..")
            print(e)
            await mp.delete(k)
            await mp.delete(message)
            pass
...
async def yt_play_list(client, m: Message):
    ...
    if m.reply_to_message is not None and m.reply_to_message.document:
        ...
        try:
            ...
            k=await m.reply(f"حدثت أخطاء أثناء قراءة قائمة التشغيل: {e}")
            await mp.delete(k)
            return
        if len(playlist)>=25:
            tplaylist=playlist[:25]
